...
plot \"./graphs/{test_type}.txt\" using 1:3:4 with boxes lc rgb var notitle, \"\"  using ($1):($3+0.2):(sprintf(\"%3.2f\",$3)) with labels font \"Arial,8\" rotate by 90 notitle", test_type=test_type, ymax=ymax);
...
fn main() {
    let algos = ["ironed", "bump_tree", "indextree", "ego_tree", "vec_tree", "naive_tree", "id_tree"];

    plot_test_type("make", &algos, 10.);
